...
export default function FillSection({ pitId, pitStatus, fills, fillEpisodes, userId, onVoted }: {
  pitId: number; pitStatus: string; fills: FillItem[];
  fillEpisodes: Map<number, string>; userId: string | null; onVoted?: () => void;
}) {
  ...
  const handleSubmit = async () => {
    ...
    const { error: err } = await supabase.from("pit_fills").insert({
      ...
      timestamp_sec: tsSec ? parseInt(tsSec) : null,
    } as never);
    if (err) { setError(err.message); return; }
    setDesc(""); setEpId(""); setTsSec(""); setShowForm(false);
    onVoted?.();
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        {userId && pitStatus === "pending" && (
          <button onClick={() => setShowForm(!showForm)} className="text-sm text-indigo-600 hover:underline">+ 填坑</button>
        )}
      </div>
      {showForm && (
        <div className="bg-white border rounded-lg p-3 mb-3 space-y-2">
          ...
          <input type="number" value={epId} onChange={e => setEpId(e.target.value)} placeholder="引用节目 ID（可选）" className="w-full px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-indigo-400" />
          <input type="number" value={tsSec} onChange={e => setTsSec(e.target.value)} placeholder="时间戳(秒)（可选）" className="w-full px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-indigo-400" />
            ...
            <div key={f.id} className="bg-white border rounded-lg p-3 text-sm">
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1 text-xs text-gray-400">
                {f.episode_id && fillEpisodes.get(f.episode_id) && (
                  <Link href={"/episodes/" + f.episode_id} className="text-indigo-500 hover:underline">{"📄 " + fillEpisodes.get(f.episode_id)}</Link>
                )}
                {f.timestamp_sec != null && <span>{"⏱ " + formatTime(f.timestamp_sec)}</span>}
              </div>
              <div className="flex items-center gap-3 mt-2">
                <button onClick={() => voteFill(f.id, "up")} className={"text-xs " + (!userId ? "text-gray-300" : "hover:text-green-600 text-green-500")}>{"👍 " + f.up_votes}</button>
                <button onClick={() => voteFill(f.id, "down")} className={"text-xs " + (!userId ? "text-gray-300" : "hover:text-red-500 text-red-400")}>{"👎 " + f.down_votes}</button>
              </div>
            </div>
          ))}
        </div>
