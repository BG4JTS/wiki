...
export default function AdminEditor({ episodes }: Props) {
  ...
  const handleLoadEpisode = async (id: number) => {
    ...
    const { data } = await supabase
      ...
      .single();
    if (data) {
      setEpNum(String(data.episode_number));
      setTitle(data.title);
  ...
  const handleDeleteEpisode = async (id: number) => {
    if (!confirm("确定要删除此节目及其时间轴、评论？")) return;
  ...
  const handleAddTimeline = async () => {
    ...
    if (error) {
      ...
      setTlTitle("");
      setTlDescription("");
      handleLoadTimelines(parseInt(timelineEpId));
    }
    setLoading(false);
  };
  ...
  const handleDeleteTimeline = async (id: number) => {
    ...
    if (!error) handleLoadTimelines(parseInt(timelineEpId));
  };
            ...
            episodes.map((ep) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => handleLoadEpisode(ep.id)}
                    className="text-xs text-indigo-600 hover:underline"
                  >
                    编辑节目
                  </button>
                  <button
                    onClick={() => handleLoadTimelines(ep.id)}
                    className="text-xs text-indigo-600 hover:underline"
                  >
                    时间轴
                  </button>
        ...
        {tab === "episode" ? (
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-indigo-400 resize-none"
              />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1">
              ...
              <textarea
                ...
                rows={15}
                className="w-full px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-indigo-400 font-mono"
              />
            </div>
            <div className="flex gap-2">
              <button
          ...
          <div className="space-y-3">
            <div>
              ...
              <select
                value={timelineEpId}
                onChange={(e) => handleLoadTimelines(parseInt(e.target.value))}
                className="w-full px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-indigo-400"
              >
                ...
                <div className="space-y-2 mb-3 max-h-60 overflow-y-auto">
                  {timelines.length === 0 ? (
                    ...
                    timelines.map((tl) => (
                      ...
                      >
                        <div>
                          <span className="font-mono text-indigo-500 text-xs">
                            {formatTime(tl.timestamp_sec)}
                          </span>
                          <span className="ml-2">{tl.title}</span>
                        </div>
                        <button
                ...
                <div className="border-t pt-3 space-y-2">
                  <div className="flex gap-2">
                    <input
                      type="number"
                      value={tlTimestamp}
                      onChange={(e) => setTlTimestamp(e.target.value)}
                      placeholder="秒数"
                      className="w-24 px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-indigo-400"
                    />
                    <input
                      type="text"
                      value={tlTitle}
                      ...
                      placeholder="章节标题"
                      className="flex-1 px-2 py-1.5 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-indigo-400"
                    />
                  </div>
                  <input
                    type="text"
