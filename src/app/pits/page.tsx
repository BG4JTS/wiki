 ...
 if(loading) return (
  <div className="animate-fade-in space-y-4">
   ...
   <div className="flex gap-2">
    {[1,2,3,4].map(i=><div key={i} className="skeleton h-7 w-16 " />)}
   </div>
     ...
     {tabs.map(t=>(
      <button key={t.key} onClick={()=>setTab(t.key)}
       className={`text-xs px-3 py-1.5 font-medium transition-all ${
        tab===t.key ? "bg-ink-700 text-white dark:bg-ink-600 " : "bg-white text-ink-500 border-2 border-ink-300 dark:bg-[#1a1a1a] dark:border-[#525252] dark:text-ink-400 hover:border-ink-700"
       }`}>
       {t.label}
      </button>
     ))}
   ...
   {filtered.length===0 ? (
    ...
    <div className="space-y-3">
     {filtered.map(pit=>{
      ...
      return (
      <Link key={pit.id} href={"/pits/"+pit.id} className="doc-row p-4 flex items-start gap-3">
       ...
       <div className="flex items-center gap-3 text-xs text-ink-400 shrink-0">
        {pp && <span className="inline-flex items-center gap-1"><img src={pp.avatar_url||""} alt="" className="w-3.5 h-3.5 object-cover" onError={e=>{(e.target as HTMLImageElement).style.display="none"}}/>{pp.username}</span>}
