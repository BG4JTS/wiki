 ...
 if (loading) return (
  <div className="animate-fade-in space-y-4">
   ...
   <div className="flex gap-2">
    {[1, 2, 3].map(i => <div key={i} className="skeleton h-7 w-16 " />)}
   </div>
    ...
    <div className="flex gap-1">
     {tabsList.map(t => (
      <button key={t.key} onClick={() => setTab(t.key)}
       className={`text-xs px-3 py-1.5 font-medium transition-all ${tab === t.key ? "bg-ink-700 text-white dark:bg-ink-600 " : "bg-white text-ink-500 border-2 border-ink-300 dark:bg-[#1a1a1a] dark:border-[#525252] dark:text-ink-400 hover:border-ink-700"
        }`}>
       {t.label}
      </button>
     ))}
    </div>
    <select value={sort} onChange={e => setSort(e.target.value as SortKey)} className="text-xs border border-ink-300 px-3 py-1.5 bg-white text-ink-500">
   ...
   {filtered.length === 0 ? (
    ...
    <div className="space-y-3">
     {filtered.map(topic => {
      ...
      return (
       <div key={topic.id} className="doc-row p-4 flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
         ...
         {pp && (
          <span className="inline-flex items-center gap-1 text-xs text-ink-400 mt-1">
           {pp.avatar_url ? <img src={pp.avatar_url} alt="" className="w-3.5 h-3.5 object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none" }} /> : null}
           {pp.username}
          </span>
         )}
        </div>
        <div className="flex items-center gap-1 shrink-0">
         <button onClick={() => handleVote(topic.id, "up")}
          className={`px-2 py-1 text-xs font-medium transition-all ${myVote === "up" ? "bg-ink-700 text-white dark:bg-ink-600" : "bg-ink-100 text-ink-500 hover:bg-ink-200 hover:text-ink-700"
           }`}>
          👍 {topic.up_votes}
         </button>
         <button onClick={() => handleVote(topic.id, "down")}
          className={`px-2 py-1 text-xs font-medium transition-all ${myVote === "down" ? "bg-red-500 text-white" : "bg-ink-50 text-ink-400 hover:bg-red-50 hover:text-red-600"
           }`}>
          👎 {topic.down_votes}
         </button>
         {score !== 0 && (
